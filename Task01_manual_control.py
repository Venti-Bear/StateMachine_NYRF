...
class ManualControl(FiniteStateMachine):
    def __init__(self, robot: Robot):
        self.robot = robot
        forward_state, backward_state, rotate_left_state, rotate_right_state, stop_state = [MonitoredState() for _ in range(5)]
        ...
        rotate_left_state.add_transition(not_left), stop_state.add_transition(left)
        ...
        rotate_right_state.add_transition(not_right), stop_state.add_transition(right)
        ...
        backward_state.add_transition(not_down), stop_state.add_transition(down)
        ...
        forward_state.add_entering_action(lambda: robot.blink_led(Side.BOTH, cycle_duration=1.0, percent_on=0.25, begin_on=True, end_off=False))
        backward_state.add_in_state_action(backward)
        backward_state.add_entering_action(lambda: robot.blink_led(Side.BOTH, cycle_duration=1.0, percent_on=0.75, begin_on=True, end_off=False))
        rotate_right_state.add_in_state_action(rotate_right)
        rotate_right_state.add_entering_action(lambda: robot.blink_led(Side.RIGHT, cycle_duration=1.0, percent_on=0.50, begin_on=True, end_off=False))
        rotate_left_state.add_in_state_action(rotate_left)
        rotate_left_state.add_entering_action(lambda: robot.blink_led(Side.LEFT, cycle_duration=1.0, percent_on=0.50, begin_on=True, end_off=False))
        stop_state.add_in_state_action(stop)
        stop_state.add_entering_action(lambda: robot.turn_led_off(side=Side.BOTH))
